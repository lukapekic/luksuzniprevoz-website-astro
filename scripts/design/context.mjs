...
try {
  ...
  const target = targetArg ? normalizeTarget(root, targetArg) : null;
  const active = resolveActiveTheme(root, config);
  const surface = target ? discoverSurface(root, config, target) : null;
  ...
  const dataModules = system?.inventory?.dataModules ?? buildSystemSnapshot(root, config).inventory.dataModules;
  const relevantData = dataModules.filter((file) => {
    if (!surface) return true;
    if (surface === "home") return /(services|fleet|reviews|operations|contact|business|navigation)\.(ts|json)$/.test(file);
    if (surface === "private-chauffeur") return /(pricing|fleet|services|operations|contact)\.(ts|json)$/.test(file);
    if (surface === "airport-transportation") return /(pricing|fleet|services|operations|contact)\.(ts|json)$/.test(file);
    if (surface === "business-transportation") return /(clients|business|fleet|services|operations|contact)\.(ts|json)$/.test(file);
    return true;
  });
  ...
  const payload = {
    ...
    warnings: [
      ...
      ...(surface && blueprints.length === 0 ? [`No installed blueprint discovered for surface "${surface}".`] : [])
    ]
  };
  ...
  if (jsonMode) {
    ...
    console.log(`Design snapshot: ${payload.designSnapshot.exists ? (payload.designSnapshot.current ? "current" : "STALE") : "MISSING"}`);
    console.log("\nAuthority:");
    payload.authorityOrder.forEach((item, index) => console.log(`  ${index + 1}. ${item}`));
    console.log("\nRead:");
    for (const file of [...payload.authorities, ...payload.blueprints, ...payload.contracts, ...payload.directImports]) console.log(`  - ${file}`);
