...
try {
  ...
  // Workers and native Node tests share this module without JSON import syntax.
  const tokensTarget = path.join(root, ".design", "tokens.ts");
  const tokensSource = `// Generated by pnpm design:sync. Do not edit manually.\nexport const tokens = ${JSON.stringify(fresh.tokens, null, 2)} as const;\n`;
  let tokensCurrent = null;
  try { tokensCurrent = readFileSync(tokensTarget, "utf8"); } catch { /* First generation. */ }
  const tokensMatch = tokensCurrent === tokensSource;
  const ok = snapshotsMatch(current, fresh) && tokensMatch;
  ...
  writeFileSync(tokensTarget, tokensSource, "utf8");
