...
export const rule = {
  ...
  description: "Production pages must use reviewed SiteHeader/SiteFooter chrome rather than the generic foundation scaffold.",
  severity: "P1",
  scan({ root, files }) {
    const findings = [];
    ...
    for (const file of files) {
      ...
      if (/SiteHeader/.test(text) && /SkipLink/.test(text) && /import\s+SkipLink/.test(text)) {
        findings.push(makeFinding({
          ...
          file: rel(root, file),
          line: 1,
          message: "This file references both SiteHeader and SkipLink; verify that the page does not emit two skip links.",
