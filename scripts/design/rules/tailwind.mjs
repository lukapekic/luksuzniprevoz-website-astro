...
const forbiddenPalette = /\b(?:bg|text|border|ring|outline|fill|stroke)-(?:slate|gray|zinc|neutral|stone|red|orange|amber|yellow|lime|green|emerald|teal|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose)-\d{2,3}\b/g;
const arbitrary = /\b(?:bg|text|border|ring|outline|shadow|rounded|max-w|min-w|max-h|min-h|w|h|gap|p[trblxy]?|m[trblxy]?)-\[([^\]]+)\]/g;
    ...
    for (const file of files) {
      ...
      while ((match = arbitrary.exec(text))) {
        ...
        findings.push(makeFinding({
          ...
          recommendation: "Map the value to a semantic theme token or an approved component contract."
        }));
      }
    }
    return findings;
