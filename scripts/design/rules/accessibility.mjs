    ...
    for (const file of files) {
      ...
      while ((match = outlineRe.exec(text))) {
        ...
        if (!/focus(?:-visible)?:/.test(line)) {
          findings.push(makeFinding({
            ...
            recommendation: "Keep a visible keyboard focus state using the semantic focus tokens."
          }));
        }
      }
      ...
      while ((match = divClickRe.exec(text))) {
        findings.push(makeFinding({
          ...
          recommendation: "Use a semantic button/link, or supply complete keyboard semantics only when no native element fits."
        }));
      }
    }
    return findings;
